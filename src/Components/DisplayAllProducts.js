    ...
    const handleDelete=(rowData)=>{
        Swal.fire({
            ...
            if (result.isConfirmed) {
                var response = await postData('products/delete_product', { productid: rowData.productid,oldpicture:rowData.picture})
                if (response.status) {
                    Swal.fire(
                        'Deleted!',
        ...
        return (<MaterialTable
            title={
                <div style={{display:'flex'}}>
                    <div><img src={categoryicon} height={35} width={35} alt='categoryicon'></img></div>
                    <div style={{ marginLeft: 5, fontFamily: 'consolas', fontSize: 22 }}>product List</div>
                </div>
            }
            columns={[
